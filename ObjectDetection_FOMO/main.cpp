 ...
 * @file     main.cpp
 * @version  V1.00
 * @brief    FOMO person-counting network sample (adapted from YOLOv8n OD).
 *
 * Inference path:
 *   CCAP camera -> RGB565 framebuffer -> imlib_nvt_scale to 192x192 RGB888
 *   in input tensor -> int8 shift (pixel - 128) -> NPU inference -> 6x6
 *   int8 sigmoid heatmap -> dequantize + 3x3 NMS + threshold -> count.
 *
...
#include "FOMOModel.hpp"          /* Model API */
...
typedef struct
{
    ...
    image_t frameImage;
    arm::app::fomo::FOMOResult result;
} S_FRAMEBUF;
...
/* Render the FOMO count as an overlay on the displayed framebuffer.
 * Also draws a small filled rectangle at every active grid cell so you
 * can see *where* the detections are. */
static void DrawFomoOverlay(
    const arm::app::fomo::FOMOResult &result,
    image_t *drawImg,
    int netRows,
    int netCols,
    float threshold
...
{
    char szText[32];
    int textColor = COLOR_R5_G6_B5_TO_RGB565(COLOR_R5_MAX, 0, 0);
    int boxColor  = COLOR_R5_G6_B5_TO_RGB565(0, COLOR_G6_MAX, 0);

    /* Top-left count overlay. */
    snprintf(szText, sizeof(szText), "people=%d", result.count);
    imlib_draw_string(drawImg, 4, 4, szText, textColor, 2, 0, 0, false,
                      false, false, false, 0, false, false);

    if (result.gridSize <= 0) return;

    /* Map the active grid cells back to display pixel coords. */
    const int g = result.gridSize;
    const int cellW = drawImg->w / g;
    const int cellH = drawImg->h / g;

    (void)netRows; (void)netCols;

    for (int gy = 0; gy < g; ++gy)
    {
        for (int gx = 0; gx < g; ++gx)
        {
            float p = result.probMap[gy * g + gx];
            if (p >= threshold)
            {
...
{
#define MODEL_FILE "0:\\model_int8_vela.tflite"
...
int main()
{
    ...
    const int inputImgCols = inputShape->data[arm::app::FOMOModel::ms_inputColsIdx];
    const int inputImgRows = inputShape->data[arm::app::FOMOModel::ms_inputRowsIdx];
    const uint32_t nChannels = inputShape->data[arm::app::FOMOModel::ms_inputChannelsIdx];
...
//				signed_req_data[i] = std::min<int8_t>(INT8_MAX, std::max<int8_t>(i_data_int8, INT8_MIN));
        ...
        if (infFramebuf)
        {
			...
			u64StartCycle = pmu_get_systick_Count();
#endif
			postProcess.RunPostProcessing(infFramebuf->result);
			info("[fomo] people = %d\n", infFramebuf->result.count);
            ...
            //draw count + active cell overlay
#if defined(__PROFILE__)
            u64StartCycle = pmu_get_systick_Count();
#endif
            DrawFomoOverlay(infFramebuf->result,
